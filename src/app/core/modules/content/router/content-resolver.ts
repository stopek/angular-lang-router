...
export class ContentResolver implements Resolve<any> {
  constructor(
    ...
    readonly selector: SelectorResolver,
  ...
  static create<T = any>(
    source: string,
    file: string,
    providedIn: 'root' | Type<T> = 'root'
  ) {
    return new InjectionToken(`content.${file}`, {
      providedIn,
      factory: () =>
        new ContentResolver(
          inject(ContentLoader as any),
          inject(SelectorResolver),
          source,
          file
        ),
    });
  ...
  public resolve(route: ActivatedRouteSnapshot): Observable<any> {
    ...
    return this.loader.loadFile(this.source, lang, this.file).pipe(
