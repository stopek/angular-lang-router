...
export class ContentSelector implements CanActivate {
  ...
  canActivate(
    ...
    const selected = this.valueAllowed(
      requested === 'auto' ? this.browserLanguage : requested
    );
    ...
    return (requested === selected ||
      this.router.createUrlTree([selected])) as AllowedContent;
  }
  ...
  public valueAllowed(value: string): string {
    return (
      this.config.supportedValues.find((allowed) => allowed === value) ||
